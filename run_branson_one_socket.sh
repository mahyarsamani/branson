...
for input in ${inputs[@]}; do
    ...
    export PAPI_OUTPUT_DIRECTORY=${PWD}/../data/one-socket/branson/$input/backend0_data
    echo "backend0"
    ./build/BRANSON inputs/$input
    export PAPI_EVENTS="PAPI_L2_TCR,PAPI_L2_TCW,PAPI_L2_TCM,PAPI_L2_TCA,PAPI_L3_DCM,PAPI_L3_TCA"
    export PAPI_OUTPUT_DIRECTORY=${PWD}/../data/one-socket/branson/$input/backend1_data
    echo "backend1"
    ./build/BRANSON inputs/$input
    export PAPI_EVENTS="PAPI_L1_ICM,PAPI_L1_ICH,PAPI_L1_ICA,PAPI_TLB_IM"
    export PAPI_OUTPUT_DIRECTORY=${PWD}/../data/one-socket/branson/$input/frontend_data
    echo "frontend"
    ./build/BRANSON inputs/$input
    export PAPI_EVENTS="PAPI_TOT_INS,PAPI_INT_INS,PAPI_FP_INS,PAPI_LD_INS"
    export PAPI_OUTPUT_DIRECTORY=${PWD}/../data/one-socket/branson/$input/inst0_data
    echo "inst0"
    ./build/BRANSON inputs/$input
    export PAPI_EVENTS="PAPI_SR_INS,PAPI_BR_INS,PAPI_VEC_INS"
    export PAPI_OUTPUT_DIRECTORY=${PWD}/../data/one-socket/branson/$input/inst1_data
    echo "inst1"
    ./build/BRANSON inputs/$input
    export PAPI_EVENTS="PAPI_STL_ICY,PAPI_STL_CCY,PAPI_BR_MSP,PAPI_BR_PRC,PAPI_RES_STL,PAPI_TOT_CYC,PAPI_LST_INS"
    export PAPI_OUTPUT_DIRECTORY=${PWD}/../data/one-socket/branson/$input/pipe0_data
    echo "pipe0"
    ./build/BRANSON inputs/$input
    export PAPI_EVENTS="PAPI_SYC_INS,PAPI_FP_OPS,PAPI_REF_CYC"
    export PAPI_OUTPUT_DIRECTORY=${PWD}/../data/one-socket/branson/$input/pipe1_data
    echo "pipe1"
    ./build/BRANSON inputs/$input
done
